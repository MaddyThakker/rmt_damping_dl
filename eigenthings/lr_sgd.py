...
parser.add_argument('--lr_freq', type=int, default=1, metavar='N',
...
parser.add_argument('--lr_warm_start', type=int, default=1, metavar='N', help='wd update frequency (default: 5)')
...
args.dir = args.dir +'/'+ args.dataset+'/'+args.model+ '/SGD_ALR'+'/warm_start='+str(args.lr_warm_start)+'_freq='+str(args.lr_freq)+'_wd='+str(args.wd)+'/'
...
for epoch in range(start_epoch, args.epochs):
    ...
    lr = args.lr_init
    learnval = lr
    if epoch % args.lr_freq == 0 and epoch> args.lr_warm_start:
        grad = utils.grad(stats_loader, model, criterion, cuda=False, bn_train_mode=False)
        top = torch.dot(grad,grad)
        bottom = torch.dot(utils.hess_vec(grad, stats_loader, model, criterion, cuda=False, bn_train_mode=False),grad)
        lr = torch.div(top,bottom)
        #lr = torch.dot(utils.grad(stats_loader, model, criterion, cuda=False, bn_train_mode=False)),utils.grad(stats_loader, model, criterion, cuda=False, bn_train_mode=False)))./torch.dot(utils.grad(utils.loss_stats(stats_loader, model, criterion, cuda=False, bn_train_mode=False),utils.hess_vec(utils.grad(stats_loader, model, criterion, cuda=False, bn_train_mode=False))))
