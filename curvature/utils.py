...
def train_epoch_tracereg(loader, model, criterion, optimizer, hess_approx, trace_factor, cuda=False, verbose=False, subset=None, backpacked_model=False,
...
def train_epoch(loader, model, criterion, optimizer, cuda=False, verbose=False, subset=None, backpacked_model=False,
                swag_model=None, swag_batch_c=64,
                *backpack_extensions):
    """
    Train the model with one pass over the entire dataset (i.e. one epoch)
...
def train_epoch_special(loader, model, criterion, optimizer, cuda=False, verbose=False, subset=None, backpacked_model=False,
                swag_model=None, swag_batch_c=64,
                *backpack_extensions):
    """
    Train the model with one pass over the entire dataset (i.e. one epoch)
...
def train_epoch_adaptive(loader, model, criterion, optimizer, cuda=False, subset=None, verbose=False):
...
def train_epoch_verbose(loader, model, criterion, optimizer, cuda=False, subset=None):
    loss_sum = 0.0
    stats_sum = defaultdict(float)
    correct_1 = 0.0
    correct_5 = 0.0
...
def eval(loader, model, criterion, cuda=False, verbose=False):
    loss_sum = 0.0
    correct_1 = 0.0
    correct_5 = 0.0
    stats_sum = defaultdict(float)
...
def hess_vec(vector, loader, model, criterion, cuda=False, bn_train_mode=True):
    param_list = list(model.parameters())
    vector_list = []
...
def hess_vec_input(vector, loader, model, criterion, cuda=False, bn_train_mode=True):
    param_list = list(model.parameters())
    vector_list = []
...
def nonggn_vec(vector, loader, model, criterion, cuda=False, bn_train_mode=True):
...
def curv_diag(loader, model, criterion, cuda=False, bn_train_mode=True, extensions=()):
...
def grad(loader, model, criterion, cuda=False, bn_train_mode=False):
    model.eval()
    if bn_train_mode:
...
def covgrad(loader, model, criterion, cuda=False, bn_train_mode=True, verbose=False):
...
def hess_noise_vec(vector, full_loader, batch_loader, model, criterion, cuda=False, bn_train_mode=True):
    """Compute the matrix-vector product between the Hessian noise matrix"""
...
def gn_vec(vector, loader, model, criterion, cuda=False, bn_train_mode=True):
    param_list = list(model.parameters())
    vector_list = []
    num_parameters = sum(p.numel() for p in param_list)
...
def gn_vec_single(vector, input, target, model, criterion, cuda=False, bn_train_mode=True):
    param_list = list(model.parameters())
    vector_list = []
    num_parameters = sum(p.numel() for p in param_list)
...
def hess_vec_single(vector, input, target, model, criterion, cuda=False, bn_train_mode=True):
    param_list = list(model.parameters())
    vector_list = []
    num_parameters = sum(p.numel() for p in param_list)
...
# code addition - 20 Nov 2019
def gn_noise_vec(vector, full_loader, batch_loader, model, criterion, cuda=False, bn_train_mode=True):
...
def shrinkage(loader, model, criterion, cuda=False, batch_loader=None, bn_train_mode=True, verbose=True):
    param_list = list(model.parameters())
    num_parameters = sum(p.numel() for p in param_list)
...
def shrinkage_grad(loader, model, criterion, cuda=False, batch_loader=None, bn_train_mode=True, verbose=True):
    param_list = list(model.parameters())
    num_parameters = sum(p.numel() for p in param_list)
...
# code modification:
def loss_stats_old(loader, model, criterion, cuda=False, bn_train_mode=True, verbose=False, curvature_matrix='hessian'):
    param_list = list(model.parameters())
    num_parameters = sum(p.numel() for p in param_list)
...
# code modification:
def loss_stats_grad(loader, model, criterion, cuda=False, bn_train_mode=True, verbose=False):
    param_list = list(model.parameters())
    num_parameters = sum(p.numel() for p in param_list)
...
def loss_stats_grad_complex(loader, model, criterion, cuda=False, bn_train_mode=True, verbose=False):
    param_list = list(model.parameters())
    num_parameters = sum(p.numel() for p in param_list)
...
def loss_stats(loader, model, criterion, cuda=False, bn_train_mode=True, verbose=False, curvature_matrix='hessian'):
    """
...
def loss_stats_layerwise(loader, model, criterion, cuda=False, bn_train_mode=True, verbose=False):
    param_list = list(model.parameters())
    num_parameters = sum(p.numel() for p in param_list)
